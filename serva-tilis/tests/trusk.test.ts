...
test(`Ask command - no id => No tool`, async () => {
  const input = `Quel est l'etat de ma commande ?`;
  const result = await agentExecutor.call({ input });

  expect(result.intermediateSteps.length).toBe(0);
});

test.skip(`Ask command - with id => Get Delivery`, async () => {
  const input = `Quel est l'etat de ma commande a5CFeW0DmA ?`;
  const result = await agentExecutor.call({ input });
...
test.skip(`Ask command - with id => Get Delivery`, async () => {
  const input = `Quand est prevue ma commande a5CFeW0DmA ?`;
  const result = await agentExecutor.call({ input });
...
test.skip(`Ask command - with id => Get Delivery`, async () => {
  const input = `Ou est mon trusker a5CFeW0DmA ?`;
  const result = await agentExecutor.call({ input });
...
test.only(`Ask command - with id => Get Delivery`, async () => {
  const input = `Ou est ma course 1342238030 ?`;
  const result = await agentExecutor.call({ input });
